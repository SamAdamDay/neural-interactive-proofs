...
solve the image classification problem on its own. Solo agents are trained using
supervised learning instead of RL.
...
def run_id_fn(arguments: RunIDFunctionArguments) -> str:
    ...
    if arguments.combo_index is None:
        return f"test_solo_ic_agents_{arguments.cmd_args.run_infix}"
    return f"test_solo_ic_agents_{arguments.cmd_args.run_infix}_{arguments.combo_index}"
