...
class TensorDictEnvironment(EnvBase, Environment, ABC):
    ...
    def _step(self, env_td: TensorDictBase) -> TensorDictBase:
        ...
        shared_done, agent_done, terminated, reward, trajectory_log_probs  = (
        ...
        next_td.set("trajectory_log_probs", trajectory_log_probs)
    ...
    def _reset(self, env_td: Optional[TensorDictBase] = None) -> TensorDictBase:
        ...
        if env_td is None or "done" not in env_td.keys():
            ...
            env_td.set("trajectory_log_probs", torch.zeros(list(env_td["y"].shape[:-1]) + [2], dtype=torch.float))
    ...
    def _masked_reset(
        ...
        env_td["trajectory_log_probs"][mask] = torch.zeros_like(env_td["trajectory_log_probs"][mask], dtype=torch.float)
