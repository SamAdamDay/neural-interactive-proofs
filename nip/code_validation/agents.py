    ...
    def build_fine_tune_dataset(
        ...
        timesteps : NestedArrayDict
            ...
            - ("next", "message_agent_id") (batch round channel): The id of the agent
              who messaged at a round-channel pair.
        ...
        ensure_last_message_is_assistant : bool, default=True
            ...
            removing messages from the end until this is true. This makes sense when
            we want to do supervised fine-tuning, as the last message should be the
            assistant's message.
    ...
    def create_dpo_fine_tune_job(
        ...
        for agent_name, agent_whole in self.agent_wholes.items():
            ...
            message_histories = agent_whole.build_fine_tune_dataset(
                timesteps_per_agent[agent_name], ensure_last_message_is_assistant=False
            )
            ...
            for i, message_history in enumerate(message_histories):
                ...
                positive_raw_message_history = positive_examples_per_agent[agent_name][
                    "next", "raw_message_history"
                ][i, :, agent_whole.agent_id]
                negative_raw_message_history = negative_examples_per_agent[agent_name][
                    "next", "raw_message_history"
                ][i, :, agent_whole.agent_id]

                # Get the ID of the round at the input rollout, and do a sanity check to
                # make sure the round ID matches between the input data and the positive
                # and negative examples
                round_id = timesteps_per_agent[agent_name][i]["round"]
                if round_id + 1 != positive_examples_per_agent[agent_name][i]["round"]:
                    raise ValueError(
                        f"Round ID mismatch between input data and positive examples "
                        f"for agent {agent_name!r} at timestep {i}. Input data has "
                        f"round ID {round_id}, positive examples have round ID "
                        f"{positive_examples_per_agent[agent_name][i]['round']}. This "
                        f"should be the round after the input data."
                    )
                if round_id + 1 != negative_examples_per_agent[agent_name][i]["round"]:
                    raise ValueError(
                        f"Round ID mismatch between input data and negative examples "
                        f"for agent {agent_name!r} at timestep {i}. Input data has "
                        f"round ID {round_id}, negative examples have round ID "
                        f"{negative_examples_per_agent[agent_name][i]['round']}. This "
                        f"should be the round after the input data."
                    )

                # Get the raw message sent by the agent in current round
                positive_message = positive_raw_message_history[round_id + 1]
                negative_message = negative_raw_message_history[round_id + 1]
                if positive_message is None:
                    raise ValueError(
                        f"Positive example message is None for agent {agent_name!r} at "
                        f"timestep {i} (round {round_id})."
                    )
