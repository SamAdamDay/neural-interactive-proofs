    ...
    for task in tqdm(range(n_tasks)):
        ...
        task_generations.extend(
            complete_code(
                pipe,
                prompt,
                num_completions=args.n_samples,
    ...
    if args.use_solutions:
        ...
        for index in range(args.num_tasks + 1):
            try:
                ...
                print(
                    f"No solutions for task {index} or not enough to have {args.n_solutions} solutions"
                )
                break
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Testing a Language Model on APPS Python Code dataset"
    )
    # model and tokenizer arguments
    parser.add_argument(
        "--model_ckpt",
        default="loubnabnl/apps-1.5B-model",
        type=str,
        help="path to model checkpoint.",
    )
    parser.add_argument(
        "--tokenizer", default="gpt2", type=str, help="tokenizer to use."
    )
    parser.add_argument(
        "--eos", default="<|endoftext|>", type=str, help="end of sentence token."
    )
    # generation arguments
    parser.add_argument(
        "--do_sample", default=True, type=bool, help="do sampling in generation"
    )
    parser.add_argument(
        "--temperature", default=0.2, type=float, help="temperature for sampling"
    )
    ...
    parser.add_argument(
        "--max_length", default=1024, type=int, help="max length of generated code"
    )
    ...
    parser.add_argument(
        ...
        default="all",
        type=str,
        help="difficulty level to select in the dataset from:\
     'all', 'introductory', 'interview'  and 'competition' ",
    )
    parser.add_argument(
        "--num_tasks", default=6, type=int, help="number of tasks to evaluate"
    )
    parser.add_argument(
        "--use_solutions",
        default=False,
        type=bool,
        help="use solutions instead of generating new code",
    )
    parser.add_argument(
        "--n_samples", default=1, type=int, help="number of samples to generate"
    )
    parser.add_argument(
        "--n_solutions", default=1, type=int, help="number of solutions to use"
    )
    parser.add_argument(
        "--k_list",
        default=[1, 2, 3],
    ...
    parser.add_argument(
        ...
        default=False,
        type=bool,
        help="count compilation and runtime errors for single generations",
    )
    # configuration
    ...
    parser.add_argument(
        ...
        default=-1,
        type=int,
        help="device on which code generation is run, if positive use GPU",
    )
    parser.add_argument("--debug", default=False, type=bool, help="debug mode")
    # save
    parser.add_argument(
        "--output_file",
        default="apps_metrics.json",
        type=str,
